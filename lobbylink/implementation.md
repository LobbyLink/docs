...
After reading and following the [prerequisites](./prerequisites) and enabling the plugin it is very simple to create your own WebRTC connection. There is a minimal example in the plugin files but I will explain the same approach here.


## Config 

You first have to configure your cloud services in the `config.cfg`. It is pretty self expanitory. This is the default configuration with my running instance configured. Note that I could at any point in time delete the server, change it's IP or chage the credentials, if the load is to high or if this connection gets misused. In a best case scenario this config should be downloaded from a server instead of hard coded into the application. This way you can change the connection without shipping out new clients. This functionality will be introduced in a future Release of LobbyLink.
